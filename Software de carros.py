# Software de peças de carros - Para professor Alex Unip
#Trabalho feito por, João Pedro, Pedro Henrique, Laelcio Ribeiro alves, Gustavo Ramos e Pedro Henrique
...
def editar_peca():
    ...
    if estoque:
        try:
            indice = int(input("Digite o número da peça que deseja editar: ")) - 1
            if 0 <= indice < len(estoque):
                peca = estoque[indice]
                print(f"Editando {peca['nome']}")
                peca['nome'] = input("Novo nome da peça: ")
                peca['quantidade'] = int(input(f"Nova quantidade de {peca['nome']}: "))
                novo_valor = float(input(f"Novo valor unitário de {peca['nome']}: "))

                peca['valor'], desconto = calcular_desconto(novo_valor)
                peca['valor_com_desconto'] = novo_valor - desconto
                

                print(f"Peça editada com sucesso! Novo valor com desconto: R${peca['valor_com_desconto']:.2f}")
            else:
...
def excluir_peca():
    ...
    if estoque:
        try:
            indice = int(input("Digite o número da peça que deseja excluir: ")) - 1
            if 0 <= indice < len(estoque):
...
def simular_compra():
    ...
    try:
        ...
        if 0 <= indice < len(estoque):
            peca = estoque[indice]
            quantidade_desejada = int(input(f"Quantas unidades de {peca['nome']} deseja comprar? "))
            ...
            if quantidade_desejada > peca['quantidade']:
                ...
                print(f"Valor unitário (com desconto): R$ {peca['valor_com_desconto']:.2f}")
...
#1 - Funcionários podem inserir, alterar e excluir os dados do estoque(PRONTO)
#2 - uando o cliente fazer uma compra, o sistema calcule o percentual de 5% de desconto, para o cliente(PRONTO)
#3 - O sistema precisa mostrar para o cliente o valor sem desconto e o valor da peça com desconto(PRONTO)
#4 - o sistema precisa mostrar a quantidade total da peças em estoque(PRONTO)
#5 - O sistema precisa calcular o total de vendas realizadas e converter os valores em porcentagem(PRONTO)
#6 - O sistema precisa calcular, quantas vendas foram realizzdas no dia na semana no mês e no ano(PRONTO)
